...
LANGUAGES["ru-ru"] = {
  ...
  vesDescription: "Эта система реализует модель CTS и обеспечивает её соблюдение. VES отвечает за загрузку и исполнение программ, написанных для CLI. Она обеспечивает исполнение управляемого кода, используя метаданные для связывания отдельных модулей. VES также называют движком исполнения",
  ...
  ctsImplementationDescription: "CTS описывает, какие типы должна поддерживать VES, и как именно она должна с ними работать. Следовательно, любая реализация VES реализует и CTS",

  clsImplementationTitle: "Любой CLI-совместимый язык программирования",
  clsImplementationDescription: "Язык программирования должен удовлетворять CLS, чтобы быть совместимым с CLI. Следовательно, любой CLI-совместимый язык реализует CLS",
}
